...
function buildExpectedCtr(rows: GscRow[]): ExpectedCtr {
  ...
  for (const [key, value] of buckets) {
    result.set(key, value.impressions > 0 ? (value.clicks / value.impressions) * 100 : fallback);
  }
...
function buildSignals(rows: PerformanceRow[], expectedCtr: ExpectedCtr, thresholds: AnalysisThresholds) {
  ...
  const growth = eligible
    .filter((row) => row.clickDelta >= thresholds.growthPercent)
    .sort((a, b) => b.clickDelta - a.clickDelta)
    .slice(0, 100)
    .map((row) => toSignal(row, `クリック+${row.clickDelta.toFixed(1)}%、表示回数${row.impressionDelta >= 0 ? '+' : ''}${row.impressionDelta.toFixed(1)}%。成長トレンドを検出。`, row.position > 10 ? 'TOP10施策を追加' : '関連テーマを拡張', `+${row.clickDelta.toFixed(1)}% clicks`));

  const declines = eligible
    .filter((row) => row.clickDelta <= thresholds.declinePercent || row.positionDelta >= 1.5)
    .sort((a, b) => a.clickDelta - b.clickDelta)
    .slice(0, 100)
    .map((row) => toSignal(row, `クリック${row.clickDelta.toFixed(1)}%、順位変化${row.positionDelta >= 0 ? '+' : ''}${row.positionDelta.toFixed(1)}。順位・CTR・需要の変化を確認。`, row.positionDelta >= 1.5 ? 'SERP差分を確認' : '需要とCTRを確認', `${row.clickDelta.toFixed(1)}% clicks`));
...
function buildOpportunities(rows: PerformanceRow[], expectedCtr: ExpectedCtr, thresholds: AnalysisThresholds): Opportunity[] {
  ...
  const add = (row: PerformanceRow, type: Opportunity['type'], scoreAdjust: number, title: string, reason: string, action: string, effort: Opportunity['effort']) => {
    candidates.push({
      ...
      reason,
      action,
      score: Math.min(100, Math.max(1, row.score + scoreAdjust)),
      impact: impactFromImpressions(row.impressions, maxImpressions),
      effort,
      impressions: row.impressions,
      position: row.position,
      delta: row.clickDelta,
  ...
  for (const row of rows.filter((item) => item.impressions >= thresholds.minImpressions)) {
    const expected = expectedCtrFor(row.position, expectedCtr);
    if (row.position >= 8 && row.position <= thresholds.opportunityMaxPosition) {
      add(row, '順位改善', 4, `${row.position.toFixed(1)}位からTOP10を狙う`, `表示回数${format(row.impressions)}。現在${row.position.toFixed(1)}位で、上位化した場合の増分余地が大きい候補です。`, '不足見出し・内部リンク・一次情報を追加', '中');
    }
    if (row.position <= thresholds.opportunityMaxPosition && row.ctr < expected * 0.75) {
      add(row, 'CTR改善', 6, `検索結果のCTRを改善`, `自サイトの同順位帯CTR基準${expected.toFixed(2)}%に対して${row.ctr.toFixed(2)}%。`, 'title / descriptionをSERP競合と比較して改善', '低');
    }
    if (row.clickDelta >= thresholds.growthPercent) {
      add(row, '成長', 2, `伸びているテーマを拡張`, `クリック+${row.clickDelta.toFixed(1)}%、表示回数${row.impressionDelta >= 0 ? '+' : ''}${row.impressionDelta.toFixed(1)}%。`, '関連クエリ・関連記事・内部リンクを追加', '低');
    }
    if (row.clickDelta <= thresholds.declinePercent || row.positionDelta >= 1.5) {
      add(row, '回復', 8, `下落を早期回復`, `クリック${row.clickDelta.toFixed(1)}%、順位変化${row.positionDelta >= 0 ? '+' : ''}${row.positionDelta.toFixed(1)}。`, 'SERP変化・競合・内容鮮度を確認してリライト', '中');
    }
    if (row.label.startsWith('http') && row.position >= 10 && row.position <= thresholds.opportunityMaxPosition) {
      add(row, '内部リンク', 1, `内部リンクで評価を集める`, `${row.position.toFixed(1)}位・表示回数${format(row.impressions)}。リンク追加だけで改善できる可能性があります。`, '関連上位ページから文脈リンクを追加', '低');
    }
  }
...
function buildCannibalization(rows: GscRow[], thresholds: AnalysisThresholds): CannibalizationGroup[] {
  ...
  for (const [query, groupRows] of grouped) {
    ...
    const pages = sorted.slice(0, 5).map((row) => ({
      url: row.keys?.[1] ?? '',
      clicks: Math.round(row.clicks),
      position: round(row.position, 1),
      share: Math.round((totalClicks > 0 ? row.clicks / totalClicks : row.impressions / totalImpressions) * 100),
    }));
    ...
    const recommendation = topShare < 55
      ? '複数URLの検索意図が近い可能性があります。統合または役割分担を明確化してください。'
      : '主URLは明確です。補助URLのtitle/H1と内部リンクで検索意図を分離してください。';
...
function buildDeviceDistribution(rows: GscRow[]): DeviceDistributionItem[] {
  ...
  return rows
    .filter((row) => Boolean(row.keys?.[0]))
    .map((row) => ({
      label: labels[row.keys?.[0] ?? ''] ?? row.keys?.[0] ?? 'その他',
      clicks: Math.round(row.clicks),
      value: totalClicks > 0 ? round((row.clicks / totalClicks) * 100, 1) : 0,
    }))
...
function buildDailyTasks(opportunities: Opportunity[]): DailyTask[] {
  return opportunities.slice(0, 4).map((item) => ({
    id: item.id,
    priority: item.score,
    title: item.title,
    detail: `${item.position.toFixed(1)}位 / ${format(item.impressions)} imp. / ${item.delta >= 0 ? '+' : ''}${item.delta.toFixed(1)}% clicks`,
    action: item.type === 'CTR改善' ? 'スニペット改善' : item.type === '回復' ? '原因を確認' : item.type === '内部リンク' ? 'リンク追加' : '詳しく見る',
  }));
}
