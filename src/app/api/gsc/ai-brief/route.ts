...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!openAiResponse.ok) return NextResponse.json({ error: openAiErrorMessage(data, `OpenAI API request failed (${openAiResponse.status})`) }, { status: openAiResponse.status });
    const brief = extractResponseText(data);
    if (!brief) return NextResponse.json({ error: 'AI response did not contain text.' }, { status: 502 });

    return NextResponse.json({ configured: true, model: process.env.OPENAI_MODEL || 'gpt-5.6-luna', searchType: analysis.searchType, brief });
  } catch (error) {
